...
def normalize_public_url(value: str) -> str:
    ...
    return f"{base}/tools/search-claim-context"
...
def build_tool_config(public_url: str, tool_token: str | None) -> dict[str, object]:
    ...
    return {
        "tool_config": {
            "type": "webhook",
            "name": "search_claim_context",
            "description": (
                "Looks up narrow live claim context only: weather, traffic, roadworks, closures, "
                "public events, or location context near the reported accident. Never use for "
                "coverage, liability, legal, premium, policy, or fraud decisions."
            ),
            ...
            "api_schema": {
                ...
                "request_body_schema": {
                    "type": "object",
                    "required": ["query"],
                    "description": "Narrow context lookup for accident weather/traffic/road conditions.",
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "One concise search query about weather, traffic, roadworks, closures, public events, or location context.",
                        },
                        "location": {
                            "type": "string",
                            "description": "Incident location or road name if known.",
                        },
                        "incident_time": {
                            "type": "string",
                            "description": "Incident time or date if known.",
                        },
                    },
                },
...
def redact_payload(value):
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if key.casefold() in {"x-tool-token", "authorization", "api-key", "xi-api-key"}:
                redacted[key] = "SET"
            else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create the ElevenLabs search_claim_context server tool.")
    ...
    try:
        ...
        payload = build_tool_config(public_url, env_first("TAVILY_TOOL_TOKEN"))
        print("Plan")
        print(f"  Tool name: {payload['tool_config']['name']}")
        print(f"  Webhook URL: {public_url}")
        print(f"  Tool token header: {'SET' if env_first('TAVILY_TOOL_TOKEN') else 'not set'}")
        if not args.apply:
        ...
        print("Attach this tool ID to the Stefanie agent or Gap Fill workflow node.")
        return 0
